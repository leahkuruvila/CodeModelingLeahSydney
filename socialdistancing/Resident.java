...
public abstract class Resident {
	 ...
	 */
	public Resident(SimulationController ctl) {
		// Settings
		this.ctl = ctl;
	...
	private void init() {
		...
		// randomize the position of the Person object to be within the SocialDistance
		// frame!
		x = (int) (Math.random() * (xExt) + 0);
		y = (int) (Math.random() * (yExt) + 0);

		// social distancing part - when do the objects have movement vx and vy?
		// for instance 10% of the time the code below will run to give the objects a
		// non-zero vx and vy
		// to simulate that 10% of instantiated objects are NOT practicing social
		// distancing
	...
	/**
	 * Velocity of an object is conditioned by settings in constructor 1. Move x, y
	 * position of resident object 2. Check if resident object boundary/frame extent
	 * is reached, then reflect
	 */
