...
public class SimulationController {
	...
	BuildingView view; // JPanel graphics window
	 ...
	 */
	public SimulationController(Settings sets, CommunityModel model) {
		// health settings
	...
	public void runSimulation() {
		...
		BuildingView view = new BuildingView(this, title, communityModel);
	...
	public void personToWallCollision(Person p) {
		...
		int numWalls = communityModel.walls.size();
		Wall wall;
		Rectangle bound;
		for (int i = 0; i < numWalls; i++) {
			wall = communityModel.walls.get(i);
			bound = communityModel.wall_boundaries.get(i);
			if (bound.intersects(personRect))
				if (wall.vertical) {
					p.vx *= -1;
				} else
					p.vy *= -1;
